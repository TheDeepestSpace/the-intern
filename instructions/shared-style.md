...
- Don't pre-check `permissions.push`/`permissions.pull` from `gh api /repos/{owner}/{repo}` as a gate before pushing or opening a PR — for GitHub App installation tokens that field can be stale and report `false` even when the token can actually push. Just attempt the `git push`/`gh pr create`; only treat it as a real permissions failure if that actual call errors out.
